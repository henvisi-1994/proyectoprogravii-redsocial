...
historias.gethistoriausuario = async (req, res) => {
    ...
    const response = await conexion.query(`SELECT historias.id_historia,historias.id_usuario,historias.imagen_historia,usuario.nom_usuario,usuario.imagen_usuario FROM historias INNER JOIN usuario on historias.id_usuario = usuario.id_usuario where historias.id_usuario= ${id_usuario} order by historias.id_historia DESC
    `);
    res.status(200).json(response.rows);
}
...
historias.registro = async(req, res) => {
    ...
    const response = await conexion.query(`SELECT * FROM publicacion INNER JOIN usuario on publicacion.id_usuario = usuario.id_usuario where historias.id_historia=${id} order by publicacion.id_pub DESC`);
    res.status(200).json(response.rows);
}
